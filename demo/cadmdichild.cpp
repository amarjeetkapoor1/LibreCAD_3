...
void CadMdiChild::newDocument() {
    ...
    viewer->addBackgroundItem(std::shared_ptr<LCVDrawItem>(new GradientBackground(lc::Color(0x06/255., 0x35/255., 0x06/255.), lc::Color(0x07/255., 0x15/255., 0x11/255.))));
    auto metricGrid = std::make_shared<MetricGrid>(20, lc::Color(0x40/255., 0x48/255., 0x40/255.), lc::Color(0x80/255., 0x90/255., 0x80/255.));
    viewer->addBackgroundItem(metricGrid);

    // Snap manager
    _snapManager = std::make_shared<SnapManagerImpl>(viewer->documentRenderer(),  std::dynamic_pointer_cast<lc::Snapable>(metricGrid), 25.);

    // Add a cursor manager, Cursor will decide the ultimate position of clicked objects
    _cursor = std::make_shared<Cursor>(40, _snapManager, lc::Color(1.,0.,0.), lc::Color(0., 1., 0.));
